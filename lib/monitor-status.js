...
function monitorStatus(redisClient, fayeClient) {
    ...
    function publishStatus(newStatus) {
        return getAsync("status-recent")
            .then((oldStatusData) => {
                ...
                const execMultiAsync = promisify(redisMulti.exec).bind(
                    redisMulti
                );
    ...
    process.on("SIGINT", function (code) {
        return publishStatus({
            ...
            salesforceStreamingConnectionReason:
                "stream_consumer process interrupted",
        }).then(() => process.exit());
    });
    ...
    process.on("SIGTERM", function (code) {
        return publishStatus({
            salesforceStreamingConnectionIsUp: false,
            salesforceStreamingConnectionReason:
                "stream_consumer process terminated",
        }).then(() => process.exit());
    });
    ...
    fayeClient.on("transport:down", function () {
        return publishStatus({
            salesforceStreamingConnectionIsUp: false,
            salesforceStreamingConnectionReason:
                "Salesforce Streaming API connection is off-line",
        });
    });
    ...
    fayeClient.on("transport:up", function () {
        return publishStatus({
            ...
            salesforceStreamingConnectionReason:
                "Salesforce Streaming API is on-line",
        });
    });
}
