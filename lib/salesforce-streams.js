...
const initSalesforceApi = require("./salesforce-init-api");
const subscribeSalesforceStreams = require("./salesforce-streams-subscribe");
...
function* streamsModule(env, messageCallback, logStream) {
    ...
    if (logStream != null && typeof logStream.write !== "function") {
        throw new Error(
            "Log stream (optional) does not appear to be a stream."
        );
    }
    ...
    const salesforceTopicNames =
        env.OBSERVE_SALESFORCE_TOPIC_NAMES &&
    ...
    if (salesforceTopicNames.length === 0) {
        throw new Error(
            "Requires OBSERVE_SALESFORCE_TOPIC_NAMES containing at least one name."
        );
    }

    const forceComVersion = env.FORCE_API_VERSION || defaultForceComVersion;
    const salesforceApi = yield Episode7.call(
        initSalesforceApi,
        env,
        forceComVersion,
        logger
