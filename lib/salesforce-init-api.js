...
function* initSalesforceApi(env, forceComVersion, logger = loggers.default) {
    if (env.SALESFORCE_URL != null) {
        // Use OAuth client
        const salesforceApi = jsforceConnection(
            env.SALESFORCE_URL,
            forceComVersion
        );

        const identity = yield Episode7.call([
            salesforceApi,
            salesforceApi.identity,
        ]);
        logger(`-----> Identity via Salesforce URL: ${identity.username}`);

        return salesforceApi;
    } else if (
        env.SALESFORCE_INSTANCE_URL != null &&
        env.SALESFORCE_ACCESS_TOKEN != null
    ) {
        ...
        env.SALESFORCE_USERNAME != null &&
        env.SALESFORCE_PASSWORD != null
    ) {
        ...
        throw new Error(
            "Requires either `SALESFORCE_URL` (containing auth) or `SALESFORCE_INSTANCE_URL`+`SALESFORCE_ACCESS_TOKEN` or `SALESFORCE_USERNAME`+`SALESFORCE_PASSWORD` environment vars."
        );
    }
}
