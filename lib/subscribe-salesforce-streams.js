...
function subscribeSalesforceStreams(
    salesforceApi,
    topicNames,
    messageCallback,
    env,
    ...
    if (typeof messageCallback !== "function") {
        throw new Error(
            "Requires messageCallback function to handle each message received."
        );
    }
    ...
    const exitCallback = () => {
        logger(
            `!      Salesforce API authentication became invalid. Exiting failure.`
        );
        console.log(
            `!      Salesforce API authentication became invalid. Exiting failure.`
        );
    ...
    return Promise.all(
        topicNames.map(topicName => {
            ...
            console.log(`-----> Subscribing to Salesforce topic ${topicName}`);
...
function subscribeAndPush(
    ...
    if (replayId != null) {
        ...
        console.log(`       ⏮  Replaying from ${replayId}`);
    ...
    fayeClient.subscribe(topicName, data => {
        ...
        return Promise.resolve()
            ...
            .catch(err => {
                ...
                console.log(`!      Streaming subscription error: ${err.stack}`);
            });
    });
}
...
const LoggingExtension = function(logger) {
    this.incoming = function(message, callback) {
        logger     (`       👁‍🗨 message from Salesforce ${JSON.stringify(message, null, 4)}`);
        console.log(`       👁‍🗨 message from Salesforce ${JSON.stringify(message, null, 4)}`);
        callback(message);
    };
};
